...
public class ShortUrl extends Weibo {
     ...
     * 获取包含指定单个短链接的最新微博内容
     *
     * @param url_short 需要取得关联微博内容的短链接，需要URLencoded
     * @return
     * @throws weibo4j.model.WeiboException when Weibo service or network is unavailable
     * @version weibo4j-V2 1.0.2
    ...
    public JSONObject statusesContentUrl(Map<String, String> map) throws WeiboException {
        ...
        return client.get(WeiboConfig.getValue("baseURL") + "short_url/share/statuses.json", parList, access_token)
                .asJSONObject();
    }
    ...
    public JSONObject commentsContentUrl(Map<String, String> map) throws WeiboException {
        PostParameter[] parList = ArrayUtils.mapToArray(map);
        return client.get(WeiboConfig.getValue("baseURL") + "short_url/comment/comments.json", parList, access_token)
                .asJSONObject();
    }
