...
public class Location extends Weibo {
     ...
     * 生成一张静态的地图图片
     *
     * @param centerCoordinate 中心点坐标，经度纬度用逗号分隔，与城市代码两者必选其一，中心点坐标优先
     * @return
    ...
    public JSONObject getMapImageByCenter(String centerCoordinate) throws WeiboException {
        return client.get(WeiboConfig.getValue("baseURL") + "location/base/get_map_image.json", new PostParameter[]{
                        new PostParameter("center_coordinate", centerCoordinate)
     ...
     * 根据实际地址返回地理信息坐标
     *
     * @param address 需要获取坐标的实际地址，必须进行URLencodes
     * @return
     * @throws weibo4j.model.WeiboException when Weibo service or network is unavailable
     * @version weibo4j-V2 1.0.2
    ...
    public List<Geos> geoToAddress(String coordinate) throws WeiboException {
        return Geos.constructGeos(
                client.get(WeiboConfig.getValue("baseURL") + "location/geo/geo_to_address.json", new PostParameter[]{
    ...
    public JSONObject gpsToOffset(String coordinate) throws WeiboException {
        return client.get(WeiboConfig.getValue("baseURL") + "location/geo/gps_to_offset.json", new PostParameter[]{
                        new PostParameter("coordinate", coordinate)
                }, access_token).asJSONObject();
    }
     ...
     * 判断地理信息坐标是否是国内坐标
     *
     * @param coordinates 需要判断的坐标，格式：经度,纬度,字符标识|经度,纬度,字符标识。其中经度纬度用逗号分隔，字符标识用于返回结果中的返回值标识
    ...
    public JSONObject isDomestic(String coordinates) throws WeiboException {
        return client.get(WeiboConfig.getValue("baseURL") + "location/geo/is_domestic.json", new PostParameter[]{
    ...
    public List<Poisition> searchPoisByLocationByQ(String q) throws WeiboException {
        return Poisition.constructPois(
                client.get(WeiboConfig.getValue("baseURL") + "location/pois/search/by_location.json",
    ...
    public List<Poisition> searchPoisByLocation(Map<String, String> map) throws WeiboException {
        PostParameter[] parList = ArrayUtils.mapToArray(map);
        return Poisition.constructPois(
                client.get(WeiboConfig.getValue("baseURL") + "location/pois/search/by_location.json", parList,
                        access_token));
    }
     ...
     *
     * @param q          查询的关键词，必须进行URLencode
     * @param coordinate 查询的中心点坐标，经度纬度用逗号分隔
     * @return
     * @throws weibo4j.model.WeiboException when Weibo service or network is unavailable
     * @version weibo4j-V2 1.0.2
     * @see http://open.weibo.com/wiki/2/location/pois/search/by_geo
    ...
    public JSONObject searchPoisByGeoByCoordinate(String q, String coordinate) throws WeiboException {
        return client.get(WeiboConfig.getValue("baseURL") + "location/pois/search/by_geo.json", new PostParameter[]{
    ...
    public JSONObject searchPoisByGeoByCenname(String q, String cenname) throws WeiboException {
        return client.get(WeiboConfig.getValue("baseURL") + "location/pois/search/by_geo.json", new PostParameter[]{
                        new PostParameter("q", q),
                        new PostParameter("cenname", cenname)
                }, access_token).asJSONObject();
    }
     ...
     *
     * @param q           查询的关键词，必须进行URLencode
     * @param coordinates 查询的矩形区域坐标，第一个坐标为左上角的点，第二个为右下角，经度纬度用逗号分隔，坐标间用“|”分隔
     * @return
     * @throws weibo4j.model.WeiboException when Weibo service or network is unavailable
     * @version weibo4j-V2 1.0.2
    ...
    public List<Poisition> searchPoisByAreaByCategory(Map<String, String> map) throws WeiboException {
        PostParameter[] parList = ArrayUtils.mapToArray(map);
        return Poisition.constructPois(
                client.get(WeiboConfig.getValue("baseURL") + "location/pois/search/by_area.json", parList,
                        access_token));
    }
     ...
     * @param cityName  POI点的城市中文名称，不超过30个字符，UTF-8编码
    ...
    public Poisition addPois(String srcid, String name, String address, String cityName, String category,
        ...
        return new Poisition(
                client.post(WeiboConfig.getValue("baseURL") + "location/pois/add.json", new PostParameter[]{
     ...
     * 根据起点与终点数据查询自驾车路线信息
     *
     * @param beginPid 查询起点POI的ID，与begin_coordinate参数必选其一，begin_pid优先
     * @param endPid   查询终点POI的ID，与end_coordinate参数必选其一，end_pid优先
     * @return
     * @throws weibo4j.model.WeiboException when Weibo service or network is unavailable
     * @version weibo4j-V2 1.0.2
    ...
    public JSONObject searchDriveRouteByPid(String beginPid, String endPid) throws WeiboException {
        return client.get(WeiboConfig.getValue("baseURL") + "location/line/drive_route.json", new PostParameter[]{
                        new PostParameter("begin_pid", beginPid),
                        new PostParameter("end_pid", endPid)
                }, access_token).asJSONObject();
    }
     ...
     * 根据起点与终点数据查询自驾车路线信息
     *
     * @param beginCoordinate 查询起点的坐标，经度纬度用逗号分隔，与begin_pid参数必选其一，begin_pid优先
     * @param endCoordinate   查询终点的坐标，经度纬度用逗号分隔，与end_pid参数必选其一，end_pid优先
     * @return
     * @throws weibo4j.model.WeiboException when Weibo service or network is unavailable
     * @version weibo4j-V2 1.0.2
     * @see http://open.weibo.com/wiki/2/location/line/drive_route
     ...
     * 根据起点与终点数据查询公交乘坐路线信息
     *
     * @param beingPid 查询起点POI的ID，与begin_coordinate参数必选其一，begin_pid优先
     * @param endPid   查询终点POI的ID，与end_coordinate参数必选其一，end_pid优先
     * @return
     * @throws weibo4j.model.WeiboException when Weibo service or network is unavailable
     * @version weibo4j-V2 1.0.2
    ...
    public JSONObject searchBusRouteByPid(String beingPid, String endPid) throws WeiboException {
        return client.get(WeiboConfig.getValue("baseURL") + "location/line/bus_route.json", new PostParameter[]{
                        new PostParameter("being_pid", beingPid),
                        new PostParameter("end_pid", endPid)
                }, access_token).asJSONObject();
    }
