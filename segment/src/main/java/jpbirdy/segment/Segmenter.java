...
public class Segmenter {
    ...
    public static List<Segment> segmentWords(List<String> text, boolean searchMode) {
        ...
        for (int i = 0; i < dict.getMaxLength(); i++)
            tokens.add(new SegToken());
        ...
        for (int current = 0; current < text.size(); current++) {
            ...
            if (numTokens == 0 || tokens.get(0).getText().size() > 1) {
                jumpers.set(current, updateJumper(jumpers.get(current), baseDistance,
                        new SegToken(splitTextToWords(text.get(current)), 1, 32, new PartOfSpeech("x"))));
            }
        }
